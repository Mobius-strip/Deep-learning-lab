...
class MyViT(nn.Module):
    ...
    def forward(self, images):
        ...
        return self.mlp(out) # Map to output dimension, output catego

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Using device: ", device, f"({torch.cuda.get_device_name(device)})" if torch.cuda.is_available() else "")
model = MyViT((3, 32, 32), n_patches=8, n_blocks=6, hidden_d=8, n_heads=8, out_d=10).to(device)
N_EPOCHS = 5
LR = 0.005
...
plt.legend()
plt.savefig("loss curve q1" )
plt.close()
plt.plot(train_acc, label='training acc')
plt.plot(val_acc, label='val acc')
plt.legend()
plt.savefig("ACCURACY curve q1 " )
plt.close()


import matplotlib.pyplot as plt

def plot_heatmap(positional_embeddings):
    fig, ax = plt.subplots(figsize=(10, 10))
    #heatmap = ax.imshow(positional_embeddings.detach().numpy())
    heatmap = ax.imshow(positional_embeddings.detach().cpu().numpy())
    ax.set_xlabel('Hidden Dimension')
    ax.set_ylabel('Positional Embedding')
    ax.set_title('Heatmap of Positional Embeddings')
    plt.colorbar(heatmap)
    plt.savefig("heatmap curve q1 " )
    plt.close() 



positional_embeddings = model.positional_embeddings
plot_heatmap(positional_embeddings)    